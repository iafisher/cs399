...
\noindent encompassing four different possibilities: satisfying both Alice and Bob's policies, satisfying one or the other, or satisfying neither. The tree below illustrates this structure diagrammatically.
...
Programmers may find it useful to convert a list of facets into a regular Racket list. The following procedure accomplishes just that.
...
\texttt{reveal-grades-rec} takes in a list of grades, a list of policies which should correspond index-by-index to the list of grades (remember that \texttt{obs} requires the policy to match the facet's policy), and an argument to pass to the policy predicates.
