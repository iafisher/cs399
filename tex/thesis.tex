...
If \texttt{add-ten} were called with a variable \texttt{x} as its argument, e.g.
...
\noindent so that \texttt{(add-ten x)} would counterintuitively evaluate to 20, despite \texttt{x} being equal to 32 as far as the programmer was concerned. In actual fact, the macro system rewrites the identifier \texttt{x} in the macro to a name which it can guarantee will not collide with any existing identifier in the program, as demonstrated below.
